import aboutMeHTML from './about-me.md'
import aboutSVCHTML from './about-svc.md'
document.querySelector('#about-me').innerHTML = aboutMeHTML
document.querySelector('#about-svc').innerHTML = aboutSVCHTML
...
!(function () {
    ...
    function __getOption() {
        var scripts = __getElesByTag('script'),
    ...
    function __onWindowScroll() {
        if (!__returnTopInterval)
            __getScrollTopOffset() > 5
                ? __animateShowXReturnTop()
                : __animateHideXReturnTop()
    }
    ...
    function __addCss() {
        var x = document.createElement('div'),
            cssStr =
                '#xReturnTop{font-size:12px;position:fixed;}#xReturnTop div,#xReturnTop div a{-webkit-border-radius:5px;-moz-border-radius:5px;border-radius:5px;width:36px;height:36px;display:block;}#xReturnTop div{transition:0.3s;position:fixed;left:' +
                opt.l +
